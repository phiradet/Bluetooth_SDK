...
void HfpAPExecCmd(BTUINT8 choice)
{
	...
	case '3': // Please dial with a phone number
		...
		if (strlen(buf2) != 0)
		{
			BTUINT32 errcode=Btsdk_HFAP_Dial(s_currHFConnHdl, buf, (BTUINT16)strlen(buf2));
			printf("return code:0x%X",errcode);
			printf("return code:%d",errcode);
		}
		break;
...
void HfpExecCmd(BTUINT8 choice)
{
	...
	if (choice == '1')					//Audio AG
	{
		HfpInit();
		TestSelectRmtHFDev();			//Select Device
		TestSelectHFSvc();				//Select service
		...
		HfpAGShowMenu();				//Show disable menu		
		while (ch != 'r')
		{
			scanf(" %c", &ch);
			getchar();		
